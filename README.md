...
基于 BGE Embedding + linear head 的轻量级新闻维度标注器。对中文财经新闻在 8 个投资行为维度上打分。
...
```bash
git clone https://github.com/junduck/newsdim newsdim
cd newsdim
uv sync
```

核心依赖仅 `numpy` 和 `sentence-transformers`（BGE 模型首次运行时自动下载，约 400MB）。
...
# 批量打分
results = tagger.score_batch(["文章1...", "文章2...", "文章3..."])
for s in results:
    print(s.to_dict())
```
...
```
文本 → BGE-base-zh-v1.5 (768 dim, frozen) → Linear head (768→8) → 8维整数分数
```
...
# 2. LLM 打分（需要 API 配置）
uv run python scripts/score_news.py --limit 1000
...
tagger.score_raw(text: str) -> dict[str, float]          # 未取整的原始分数
tagger.score_batch_raw(texts: list[str]) -> list[dict[str, float]]
tagger.encoder  # BGEEncoder 实例
tagger.head     # LinearHead 实例
